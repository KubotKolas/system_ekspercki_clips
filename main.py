...
class MyWidget(QtWidgets.QWidget):
    ...
    def __init__(self, newEnv):
        ...
        #     "What can you do?",
        #     "Sweet! You're ready to build a site!",
        ...
        self.allButtons[0].clicked.connect(lambda: self.update("1"))
        self.allButtons[1].clicked.connect(lambda: self.update("2"))
        self.allButtons[2].clicked.connect(lambda: self.update("3"))
        self.allButtons[3].clicked.connect(lambda: self.update("4"))
        self.allButtons[4].clicked.connect(lambda: self.update("5"))
    ...
    def runClipsLogic(self, fact_string): # TODO: implement updating facts
        ...
        if fact_string is not None:
            answer = "(answer " + str(self.questionNr) + " " + fact_string + " \"" + self.answers[int(fact_string)-1]+"\")"
            self.env.assert_string(answer)
        ...
        for fact in self.env.facts():
            ...
            if f == "question":
                self.question = fact["text"]
                self.answers = fact["possible_answers"]
                self.questionNr = fact["nr"]
            if f == "result":
                self.finished = fact[0]
                self.question = fact[1]

    def updateButtons(self):
        self.answers = [x for x in self.answers]
        # self.buttonStates = [x for x in self.answers]
        while len(self.answers) < 5:
            self.answers.append(None)
        for i in range(5):
            self.allButtons[i].setText(self.answers[i])
            if self.answers[i] is not None:
                self.allButtons[i].setEnabled(True)
            else:
                self.allButtons[i].setEnabled(False)
